...
const Timeline = function(param = {}) {
  const _selector = document.querySelector(param.selector);
  ...
  const create = async function() {
      ...
      renderMoreFeed(await getMoreData());
      loading.style.display = 'none';
      const clientRect = _selector.getBoundingClientRect();
  ...
  const initScroll = async () => {
    ...
    // TODO 페이지가 1보다 클 때만 이벤트 붙이기 // XXX 이해 안감 
    addEvent();
  } 
